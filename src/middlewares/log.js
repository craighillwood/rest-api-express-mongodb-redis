...
const requestStream = LOG_TO_FILE === false || isTest === true ? process.stdout : rfs('request.log', {
  interval: '1d',
  path: logsPath,
});
...
const responseOkStream = LOG_TO_FILE === false || isTest === true ? process.stdout : rfs('response-ok.log', {
  interval: '1d',
  path: logsPath,
});
...
const responseErrorStream = LOG_TO_FILE === false || isTest === true ? process.stderr : rfs('response-error.log', {
  interval: '1d',
  path: logsPath,
});
